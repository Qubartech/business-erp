...
export function Modal({ open, onClose, title, children, footer }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-slate-950/60 p-4" onClick={onClose}>
      <div className="card w-full max-w-lg p-0 flex flex-col max-h-[85vh] sm:max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        {footer && <div className="border-t border-slate-200 dark:border-slate-800 px-4 py-3 flex justify-end gap-2 shrink-0">{footer}</div>}
      </div>
    </div>
  );
}
