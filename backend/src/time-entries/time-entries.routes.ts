...
import { listEntriesQuerySchema, manualEntrySchema, startTimerSchema, stopTimerSchema, updateEntrySchema } from "./time-entries.schemas.js";
...
timeEntriesRouter.post("/stop", validate(stopTimerSchema), async (req, res, next) => {
  try { ok(res, await service.stopTimer(req.user!.sub, req.body.entryId, req.body.endTime), "Timer stopped"); } catch (e) { next(e); }
});
...
timeEntriesRouter.patch("/:id", validate(updateEntrySchema), async (req, res, next) => {
  try { ok(res, await service.update(req.user!.sub, req.user!.role, req.params.id, req.body), "Entry updated"); } catch (e) { next(e); }
});
